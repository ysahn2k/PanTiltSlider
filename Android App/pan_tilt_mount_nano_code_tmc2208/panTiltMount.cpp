...
void serialData(void){
    ...
    if(instruction == INSTRUCTION_BYTES_SLIDER_PAN_TILT_SPEED){
        ...
        while(Serial.available() < 6){//Wait for 6 bytes to be available. Breaks after ~20ms if bytes are not received.
                ...
                count++;
                //if(count > 100){
                if(count > 250){ // To match the velocity of MIT APP INVENTOR extended the time to 50msec
...
//            int tiltStepSpeed = (Serial.read() << 8) + Serial.read(); 
            int sliderStepSpeed = Serial.read() + (Serial.read() << 8); // To match format of MIT APP INVENTOR exchanged the MSB(8bytes) and the LSB(8bytes)
            int panStepSpeed = Serial.read() + (Serial.read() << 8); // To match format of MIT APP INVENTOR exchanged the MSB(8bytes) and the LSB(8bytes)
            int tiltStepSpeed = Serial.read() + (Serial.read() << 8); // To match format of MIT APP INVENTOR exchanged the MSB(8bytes) and the LSB(8bytes)
